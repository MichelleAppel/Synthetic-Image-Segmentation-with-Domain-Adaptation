...
import numpy as np
import src.segmentation.models.vgg16_c as vgg16_c
...
class BDCN(pl.LightningModule):
    def __init__(self, pretrain=None, logger=None, rate=4, log_interval=5):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step % self.log_interval == 0:
            ...
            n = 4
            # log images
            images = make_grid(image[:n], normalize=True)
            labels = make_grid(label[:n], normalize=True)
            preds = make_grid(out[-1][:n], normalize=True)

            wandb.log({"images": [wandb.Image(images)]})
            wandb.log({"labels": [wandb.Image(labels)]})
            wandb.log({"preds": [wandb.Image(preds)]})
    ...
    def combined_loss(self, preds, targets):
        ...
        losses = []
        for pred in preds[:-1]:
            losses += [self.side_weight * F.binary_cross_entropy_with_logits(
                pred.float(), targets.float(), weight=weight, reduction='sum')]
        losses += [self.fuse_weight * F.binary_cross_entropy_with_logits(
            preds[-1].float(), targets.float(), weight=weight, reduction='sum')]
